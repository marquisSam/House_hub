...
const TodoStore = signalStore(
  ...
  withComputed(({ todoEntities, todoIsLoading, error }) => ({
    todosCount: () => todoEntities().length,
    hasError: () => !!error(),
    completedTodos: () => todoEntities().filter((todo: Todo) => todo.IsCompleted),
    pendingTodos: () => todoEntities().filter((todo: Todo) => !todo.IsCompleted),
  }))
);
